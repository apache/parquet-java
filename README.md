...
```
wget -nv http://archive.apache.org/dist/thrift/0.18.1/thrift-0.18.1.tar.gz
tar xzf thrift-0.18.1.tar.gz
cd thrift-0.18.1
chmod +x ./configure
...
If you're on OSX and use homebrew, you can instead install Thrift 0.18.1 with `brew` and ensure that it comes first in your `PATH`.

```
brew install thrift
export PATH="/usr/local/opt/thrift@0.18.1/bin:$PATH"
```
