...
EOF
  ...
  ( cd "$SCRIPT_PATH/.." && ./mvnw -pl parquet-benchmarks -am -DskipTests -Denforcer.skip=true clean package )
  ...
  JMH_COMMAND=(java -jar "${SCRIPT_PATH}/target/parquet-benchmarks.jar")
  if [ -n "$BENCHMARK_REGEX" ]; then
    JMH_COMMAND+=("$BENCHMARK_REGEX")
  fi
  JMH_COMMAND+=("${JMH_OPTIONS[@]}")

  printf 'JMH command:'
  printf ' %q' "${JMH_COMMAND[@]}"
  printf '\n'
  "${JMH_COMMAND[@]}"
  JMH_STATUS=$?

  # Clean any data files generated by the benchmarks without hiding JMH failures.
