...
public class TestColumnIndexBuilderNaN {
  ...
  @Test
  public void testHasNaNsWithNanCounts() {
    // Build a column index with nanCounts available
    ColumnIndexBuilder builder = ColumnIndexBuilder.getBuilder(DOUBLE_IEEE754_TYPE, Integer.MAX_VALUE);
    builder.add(doubleStats(DOUBLE_IEEE754_TYPE, 1.0, 2.0)); // page 0: no NaN
    builder.add(doubleStats(DOUBLE_IEEE754_TYPE, Double.NaN, Double.NaN)); // page 1: all NaN
    builder.add(doubleStats(DOUBLE_IEEE754_TYPE, 3.0, 4.0)); // page 2: no NaN
    ColumnIndexBuilder.ColumnIndexBase<?> ci = (ColumnIndexBuilder.ColumnIndexBase<?>) builder.build();
    assertNotNull(ci);

    // Page 0: no NaN → hasNaNs should be false
    assertFalse(ci.hasNaNs(0));
    // Page 1: all NaN → hasNaNs should be true
    assertTrue(ci.hasNaNs(1));
    // Page 2: no NaN → hasNaNs should be false
    assertFalse(ci.hasNaNs(2));
  }

  @Test
  public void testHasNaNsWithoutNanCounts() {
    // Build a column index without nanCounts by using the static build() method with null nanCounts.
    // This simulates reading a file from an older writer that didn't track nanCounts.
    java.util.List<Boolean> nullPages = java.util.List.of(false, false);
    java.util.List<Long> nullCounts = java.util.List.of(0L, 0L);
    java.util.List<java.nio.ByteBuffer> minValues = java.util.List.of(
        java.nio.ByteBuffer.wrap(new byte[] {0, 0, 0, 0, 0, 0, (byte) 0xF0, 0x3F}), // 1.0
        java.nio.ByteBuffer.wrap(new byte[] {0, 0, 0, 0, 0, 0, 8, 0x40})); // 3.0
    java.util.List<java.nio.ByteBuffer> maxValues = java.util.List.of(
        java.nio.ByteBuffer.wrap(new byte[] {0, 0, 0, 0, 0, 0, 0, 0x40}), // 2.0
        java.nio.ByteBuffer.wrap(new byte[] {0, 0, 0, 0, 0, 0, 0x10, 0x40})); // 4.0
    ColumnIndex ci = ColumnIndexBuilder.build(
        DOUBLE_TYPE,
        org.apache.parquet.internal.column.columnindex.BoundaryOrder.ASCENDING,
        nullPages,
        nullCounts,
        null,
        minValues,
        ...
        null);
    assertNotNull(ci);
    ColumnIndexBuilder.ColumnIndexBase<?> base = (ColumnIndexBuilder.ColumnIndexBase<?>) ci;

    // When nanCounts is not available, hasNaNs should conservatively return true
    assertTrue(base.hasNaNs(0));
    assertTrue(base.hasNaNs(1));
  }
}
