...
public class TestByteBitPacking {
  ...
  public void testPackUnPack() {
    ...
    for (int i = 1; i < 32; i++) {
      ...
      packUnpack(ByteBitPacking.getPacker(i), values, unpacked);
  ...
  public void testPackUnPackAgainstHandWritten() throws IOException {
    ...
    for (int i = 1; i < 8; i++) {
      ...
      // pack generated
      final BytePacker packer = ByteBitPacking.getPacker(i);
      packer.pack32Values(values, 0, packed, 0);
  ...
  public void testPackUnPackAgainstLemire() throws IOException {
    System.out.println();
    System.out.println("testPackUnPackAgainstLemire");
    for (int i = 1; i < 32; i++) {
      System.out.println("Width: " + i);
      ...
      System.out.println("Le out: " + TestBitPacking.toString(packedByLemireAsBytes));

      // pack manual
      final BytePacker bytePacker = ByteBitPacking.getPacker(i);
      byte[] packedGenerated = new byte[i * 4];
      bytePacker.pack32Values(values, 0, packedGenerated, 0);
      System.out.println("Ge out: " + TestBitPacking.toString(packedGenerated));
      Assert.assertEquals("width " + i, TestBitPacking.toString(packedByLemireAsBytes), TestBitPacking.toString(packedGenerated));
