...
/**
 * Similar to a {@link ByteArrayOutputStream}, but uses a different strategy for
 * growing that does not involve copying. Where ByteArrayOutputStream is backed
 * by a single array that "grows" by copying into a new larger array, this
 * output stream grows by allocating a new array (slab) and adding it to a list
 * of previous arrays.
 *
 * Each new slab is allocated to be the same size as all the previous slabs
 * combined, so these allocations become exponentially less frequent, just like
 * ByteArrayOutputStream, with one difference. This output stream accepts a max
 * capacity hint, which is a hint describing the max amount of data that will be
 * written to this stream. As the total size of this stream nears this max, this
 * stream starts to grow linearly instead of exponentially. So new slabs are
 * allocated to be 1/5th of the max capacity hint, instead of equal to the total
 * previous size of all slabs. This is useful because it prevents allocating
 * roughly twice the needed space when a new slab is added just before the
 * stream is done being used.
 *
 * When reusing this stream it will adjust the initial slab size based on the
 * previous data size, aiming for fewer allocations, with the assumption that a
 * similar amount of data will be written to this stream on re-use. See
 * ({@link CapacityByteArrayOutputStream#reset()}).
 */
public class CapacityByteArrayOutputStream extends OutputStream {
   ...
   * constructed with it will end up allocating targetNumSlabs in order to reach
   * targetCapacity. This aims to be a balance between the overhead of creating
   * new slabs and wasting memory by eagerly making initial slabs too big.
   *
   * Note that targetCapacity here need not match maxCapacityHint in the
   * constructor of CapacityByteArrayOutputStream, though often that would make
   * sense.
   *
   * @param minSlabSize no matter what we shouldn't make slabs any smaller than
   * this
   * @param targetCapacity after we've allocated targetNumSlabs how much capacity
   * should we have?
  ...
  public static CapacityByteArrayOutputStream withTargetNumSlabs(int minSlabSize, int maxCapacityHint,
      int targetNumSlabs) {
    return withTargetNumSlabs(minSlabSize, maxCapacityHint, targetNumSlabs, new HeapByteBufferAllocator());
  }

  /**
   * Construct a CapacityByteArrayOutputStream configured such that its initial
   * slab size is determined by {@link #initialSlabSizeHeuristic}, with
   * targetCapacity == maxCapacityHint
   *
   ...
   */
  public static CapacityByteArrayOutputStream withTargetNumSlabs(int minSlabSize, int maxCapacityHint,
      int targetNumSlabs, ByteBufferAllocator allocator) {

    return new CapacityByteArrayOutputStream(initialSlabSizeHeuristic(minSlabSize, maxCapacityHint, targetNumSlabs),
   ...
   * @deprecated use
   * {@link CapacityByteArrayOutputStream#CapacityByteArrayOutputStream(int, int, ByteBufferAllocator)}
   */
  @Deprecated
  ...
  public CapacityByteArrayOutputStream(int initialSlabSize, int maxCapacityHint, ByteBufferAllocator allocator) {
    ...
    checkArgument(maxCapacityHint >= initialSlabSize,
        String.format("maxCapacityHint can't be less than initialSlabSize %d %d", initialSlabSize, maxCapacityHint));
  ...
  private void addSlab(int minimumSize) {
    ...
    if (bytesUsed == 0) {
      ...
      // to avoid an overhead of up to twice the needed size, we get linear when
      // approaching target page size
  ...
  private void writeToOutput(OutputStream out, ByteBuffer buf, int len) throws IOException {
    if (buf.hasArray()) {
      ...
      // The OutputStream interface only takes a byte[], unfortunately this means that
      // a ByteBuffer
   ...
   * argument. the output stream's write method
   * <code>out.write(slab, 0, slab.length)</code>) will be called once per slab.
   *
   ...
   * previous data size. The intent is to reuse the same instance for the same
   * type of data (for example, the same column). The assumption is that the size
   * in the buffer will be consistent.
   */
  public void reset() {
    // readjust slab size.
    // 7 = 2^3 - 1 so that doubling the initial size 3 times will get to the same
    // size
  ...
  /**
   * @return the index of the last value written to this stream, which can be
   * passed to {@link #setByte(long, byte)} in order to change it
   */
