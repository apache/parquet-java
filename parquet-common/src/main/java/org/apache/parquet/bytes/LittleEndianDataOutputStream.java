...
public class LittleEndianDataOutputStream extends OutputStream {
  ...
  /**
   * Writes a <code>short</code> to the underlying output stream as two bytes, low
   * byte first. If no exception is thrown, the counter <code>written</code> is
  ...
  /**
   * Writes an <code>int</code> to the underlying output stream as four bytes, low
   * byte first. If no exception is thrown, the counter <code>written</code> is
   ...
   * low byte first. In no exception is thrown, the counter <code>written</code>
   ...
   * 8-byte quantity, low byte first. If no exception is thrown, the counter
