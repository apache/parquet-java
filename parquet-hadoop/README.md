...
**Property:** `parquet.block.size.row.check.min`
**Description:** The frequency of checks of the block size limit will be between
`parquet.block.size.row.check.min` and `parquet.block.size.row.check.max`.
If the frequency is high, the block size will be accurate.
If the frequency is low, the performance will be better.
**Default value:** `100`

---

**Property:** `parquet.block.size.row.check.max`
**Description:** The frequency of checks of the block size limit will be between
`parquet.block.size.row.check.min` and `parquet.block.size.row.check.max`.
If the frequency is high, the block size will be accurate.
If the frequency is low, the performance will be better.
