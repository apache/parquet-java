...
class InternalParquetRecordWriter<T> {
  private static final Logger LOGGER = LoggerFactory.getLogger(InternalParquetRecordWriter.class);
  ...
  private void checkBlockSizeReached() throws IOException {
    if (recordCount >= recordCountForNextMemCheck) { // checking the memory size is relatively expensive, so let's not do it for every record.
      ...
      if (memSize > (nextRowGroupSize - 2 * recordSize)) {
        LOGGER.info(format("mem size %,d > %,d: flushing %,d records to disk.", memSize, nextRowGroupSize, recordCount));
        flushRowGroupToStore();
        initStore();
        ...
        if (LOGGER.isDebugEnabled()){
          LOGGER.debug(format("Checked mem at %,d will check again at: %,d ", recordCount, recordCountForNextMemCheck));
        }
      }
    }
  }
