 ...
 * @param <V> The value type. Must extend
 * {@link org.apache.parquet.hadoop.LruCache.Value} so that the "staleness" of
...
final class LruCache<K, V extends LruCache.Value<K, V>> {
   ...
   * value is only inserted if it is not null and it is considered current. If the
   * cache previously contained a mapping for the key, the old value is replaced
  ...
  /**
   * Removes all of the mappings from this cache. The cache will be empty after
   * this call returns.
   */
  public void clear() {
    cacheMap.clear();
   ...
   * interface so the cache can determine 1) whether values are current (e.g. the
   * referenced data has not been modified/updated in such a way that the value is
   * no longer useful) and 2) whether a value is strictly "newer" than another
   * value.
   *
  ...
  interface Value<K, V> {
     ...
     * @return {@code true} the value is still current, {@code false} the value is
     * no longer useful
     */
     ...
     * {@code false} the value is older or just as new as the other value.
     */
