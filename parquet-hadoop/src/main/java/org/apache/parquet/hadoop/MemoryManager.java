 ...
 *
 * This class balances the allocation size of each writer by resize them
 * averagely. When the sum of each writer's allocation size is less than total
 * memory pool, keep them original value. When the sum exceeds, decrease each
 * writer's allocation size by a ratio.
 */
public class MemoryManager {
  ...
  public MemoryManager(float ratio, long minAllocation) {
    ...
    totalMemoryPool = Math.round((double) ManagementFactory.getMemoryMXBean().getHeapMemoryUsage().getMax() * ratio);
  ...
  synchronized void addWriter(InternalParquetRecordWriter writer, Long allocation) {
    ...
    if (oldValue == null) {
      ...
      throw new IllegalArgumentException("[BUG] The Parquet Memory Manager should not add an "
          + "instance of InternalParquetRecordWriter more than once. The Manager already contains " + "the writer: "
          + writer);
    }
    updateAllocation();
  }
  ...
  /**
   * Update the allocated size of each writer based on the current allocations and
   * pool size.
   */
  private void updateAllocation() {
    ...
    for (Map.Entry<InternalParquetRecordWriter, Long> entry : writerList.entrySet()) {
      ...
      if (scale < 1.0 && minMemoryAllocation > 0 && newSize < minMemoryAllocation) {
        throw new ParquetRuntimeException(String.format(
            "New Memory allocation %d bytes" + " is smaller than the minimum allocation size of %d bytes.", newSize,
            minMemoryAllocation)) {
        };
      }
      entry.getKey().setRowGroupSizeThreshold(newSize);
      LOG.debug(
          String.format("Adjust block size from %,d to %,d for writer: %s", entry.getValue(), newSize, entry.getKey()));
    }
  }
