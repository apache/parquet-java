...
class RowGroupMerger {
  ...
  RowGroupMerger(MessageType schema, CompressionCodecName compression, boolean useV2ValueWriter) {
   ...
   */
  void merge(List<InputFile> inputFiles, final long maxRowGroupSize, ParquetFileWriter writer) throws IOException {
  ...
  private static BiConsumer<ValuesReader, ValuesWriter> createWritingBridge(PrimitiveType.PrimitiveTypeName typeName) {
    switch (typeName) {
