 ...
 *       complete encrypted module. The AAD uses the {@code Self-Reference} module type (10) with an
 *       8-byte self-reference ordinal; see {@link AesCipher#createSelfReferenceAAD}.
 * </ul>
 *
...
public final class SelfReferenceStorage {
  ...
  public static StoredRange write(
    ...
    // independent module. The encryptor prepends the 4-byte length and the nonce and appends the
    // GCM tag (for AES_GCM_V1); the returned byte array is the complete stored module.
    if (pageBlockEncryptor != null) {
      byte[] selfReferenceAAD =
          AesCipher.createSelfReferenceAAD(fileAAD, rowGroupOrdinal, columnOrdinal, selfReferenceOrdinal);
  ...
  public static BytesInput resolve(
    ...
    // length, nonce, ciphertext, and GCM tag and returns the compressed block.
    ...
    // Step 2: decompress. The resolved size is not stored, so the codec stream is drained to EOF.
