...
/**
 * Tracks number of records that cannot be materialized and throws
 * ParquetDecodingException if the rate of errors crosses a limit.
 * <p>
 * These types of errors are meant to be recoverable record conversion errors,
 * such as a union missing a value, or schema mismatch and so on. It's not meant
 * to recover from corruptions in the parquet columns themselves.
 *
 * The intention is to skip over very rare file corruption or bugs where the
 * write path has allowed invalid records into the file, but still catch large
 * numbers of failures. Not turned on by default (by default, no errors are
 * tolerated).
 */
public class UnmaterializableRecordCounter {
  ...
  public void incErrors(RecordMaterializationException cause) throws ParquetDecodingException {
    ...
    LOG.warn(String.format("Error while reading an input record (%s out of %s): ", numErrors, totalNumRecords), cause);
