...
public class TestDirectCodecFactory {
  ...
  public void directFactoryLeveledZstdRoundTrip() throws IOException {
    ...
    try {
      ...
      Assert.assertTrue(
          "heap factory should produce a heap ZstdBytesCompressor",
          heap.getCompressor(ZSTD, 3) instanceof CodecFactory.ZstdBytesCompressor);
      ...
      Assert.assertFalse(
          "direct factory ZSTD(level) should not fall back to the heap compressor",
          directZstd instanceof CodecFactory.ZstdBytesCompressor);
      ...
      Assert.assertFalse(
          "direct factory SNAPPY(level) should not fall back to the heap compressor",
          directSnappy instanceof CodecFactory.SnappyBytesCompressor);
      Assert.assertEquals(
