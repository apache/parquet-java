      ...
      fileReader.getRowGroups().forEach(block -> {
        ...
        block.getColumns().stream()
          ...
          .forEach(column -> {
            ...
            // set 10 candidates:
            // [bitSize=8192, expectedNVD=500], [bitSize=16384, expectedNVD=1500] , [bitSize=32768, expectedNVD=3000],
            // [bitSize=65536, expectedNVD=6500] ........
            // number of distinct values is less than 100, so the bitSize should be less than 131073.
            assertTrue(bitsetSize <= 8192);
          });
      });
    }
  }
