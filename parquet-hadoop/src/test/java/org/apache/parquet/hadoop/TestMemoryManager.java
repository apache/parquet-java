...
public class TestMemoryManager {
  ...
  public void setUp() throws Exception {
    ...
    expectedPoolSize = Math.round((double) ManagementFactory.getMemoryMXBean().getHeapMemoryUsage().getMax()
  ...
  public void testMemoryManagerUpperLimit() {
    ...
    Assert.assertTrue("Pool size should be within 10% of the expected value" + " (expected = " + expectedPoolSize
        + " actual = " + poolSize + ")", Math.abs(expectedPoolSize - poolSize) < (long) (expectedPoolSize * 0.10));
  }
  ...
  public void testMemoryManager() throws Exception {
    ...
    Assert.assertTrue("Allocations should never exceed pool size", getTotalAllocation() <= poolSize);
    Assert.assertEquals("Second writer should be limited by row group size", 2 * rowGroupSize, getTotalAllocation());
    ...
    writer2.close(null);
    Assert.assertTrue("Allocations should never exceed pool size", getTotalAllocation() <= poolSize);
    Assert.assertEquals("Allocations should be increased to the row group size", rowGroupSize, getTotalAllocation());
  ...
  private RecordWriter createWriter(int index) throws Exception {
    ...
    RecordWriter writer = parquetOutputFormat.getRecordWriter(conf, new Path(file.toString()),
  ...
  private long getTotalAllocation() {
    Set<InternalParquetRecordWriter> writers = ParquetOutputFormat.getMemoryManager().getWriterList().keySet();
    long total = 0;
