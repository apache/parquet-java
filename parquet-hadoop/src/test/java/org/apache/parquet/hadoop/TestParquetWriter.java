...
public class TestParquetWriter {
  ...
  public void test() throws Exception {
    ...
    for (int modulo : asList(10, 1000)) {
      for (WriterVersion version : WriterVersion.values()) {
        ...
        assertEquals("Object model property should be example", "example",
    ...
    TestUtils.assertThrows("Should reject a schema with an empty group", InvalidSchemaException.class,
        (Callable<Void>) () -> {
          ExampleParquetWriter.builder(new Path(file.toString()))
              .withType(
                  Types.buildMessage().addField(new GroupType(REQUIRED, "invalid_group")).named("invalid_message"))
              .build();
          return null;
        });
  ...
  public void testNullValuesWithPageRowLimit() throws IOException {
    ...
    try (ParquetWriter<Group> writer = ExampleParquetWriter.builder(path).withPageRowCountLimit(10).withConf(conf)
        .build()) {
