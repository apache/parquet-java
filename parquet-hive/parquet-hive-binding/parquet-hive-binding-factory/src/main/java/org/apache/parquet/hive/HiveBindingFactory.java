 ...
 * available in the classpath. This class does not provide static methods to
 * enable mocking.
 */
public class HiveBindingFactory {
  ...
  public HiveBinding create() {
    Class<? extends HiveBinding> bindingClazz = create(HiveBindingFactory.class.getClassLoader());
    try {
      return bindingClazz.newInstance();
    } catch (Exception e) {
      throw new HiveBindingInstantiationError(
          "Unexpected error creating instance" + " of " + bindingClazz.getCanonicalName(), e);
    }
  }
  ...
  Class<? extends HiveBinding> create(ClassLoader classLoader) {
    ...
    try {
      ...
      LOG.debug("Class " + HIVE_VERSION_CLASS_NAME + ", not found, returning {}", Hive010Binding.class.getSimpleName());
  ...
  Class<? extends HiveBinding> createInternal(Class hiveVersionInfo) {
    ...
    if (hiveVersion.startsWith(HIVE_VERSION_010)) {
      ...
      LOG.debug("Hive version " + hiveVersion + ", returning " + Hive010Binding.class.getSimpleName()
          + " as it's expected the 0.10 " + "binding will work with 0.11");
      return Hive010Binding.class;
      ...
      throw new HiveBindingInstantiationError("Hive 0.13 contains native Parquet support "
          + "and the parquet-hive jars from the parquet project should not be included " + "in Hive's classpath.");
    }
    LOG.debug("Hive version {}, returning {}", hiveVersion, Hive012Binding.class.getSimpleName());
    ...
    try {
      ...
      for (Method method : utilitiesClass.getDeclaredMethods()) {
        if (HIVE_012_INDICATOR_UTILITIES_GETMAPWORK.equals(method.getName())) {
          LOG.debug("Found " + HIVE_UTILITIES_CLASS_NAME + "." + HIVE_012_INDICATOR_UTILITIES_GETMAPWORK
              + " returning 0.12 binding");
          return Hive012Binding.class;
        }
      }
