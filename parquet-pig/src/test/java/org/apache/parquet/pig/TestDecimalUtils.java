...
public class TestDecimalUtils {
  ...
  private void testDecimalConversion(double value, int precision, int scale, String stringValue) {
    ...
    BigDecimal convertedValue = DecimalUtils
        .binaryToDecimal(Binary.fromByteArray(originalValue.unscaledValue().toByteArray()), precision, scale);
  ...
  public void testBinaryToDecimal() throws Exception {
    // Known issue: testing Nx10^M doubles from BigDecimal.unscaledValue() always
    // converts to Nx10 regardless of M
    // Known issue: any double with precision > 17 breaks in test but not in
    // functional testing
