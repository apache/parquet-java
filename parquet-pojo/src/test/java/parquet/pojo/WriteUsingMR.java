...
public class WriteUsingMR {
  ...
  private static List inputMessages;
  Path outputPath;

  public static Path someTemporaryFilePath() throws IOException {
    File tmp = File.createTempFile("ParquetPojo_unitTest", ".tmp");
    tmp.deleteOnExit();
    tmp.delete();
  ...
  public static class WritingMapper extends Mapper<LongWritable, Text, Void, Object> {

    public void run(Context context) throws IOException, InterruptedException {
      if (inputMessages == null || inputMessages.size() == 0) {
        throw new RuntimeException("No mock data given");
      } else {
        for (Object msg : inputMessages) {
  ...
  public <T> Path write(List items, Class<T> tClass, Class... genericArguments) throws Exception {

    synchronized (WriteUsingMR.class) {

      outputPath = someTemporaryFilePath();

      Path inputPath = someTemporaryFilePath();
      ...
      ParquetPojoOutputFormat.setOutputPath(job, outputPath);

      if (Map.class.isAssignableFrom(tClass)) {
        ParquetPojoOutputFormat.setMapSchemaClass(
          job, (Class) tClass, genericArguments[0], genericArguments[1]
        );
      } else if (List.class.isAssignableFrom(tClass)) {
        ParquetPojoOutputFormat.setListSchemaClass(job, (Class) tClass, genericArguments[0]);
      } else {
        ParquetPojoOutputFormat.setSchemaClass(job, tClass);
      }
