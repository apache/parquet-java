...
/**
 * Converts Protocol Buffer message (both top level and inner) to parquet. This
 * is internal class, use {@link ProtoRecordConverter}.
 */
class ProtoMessageConverter extends GroupConverter {
  ...
  private Converter newMessageConverter(final Message.Builder parentBuilder,
  ...
  final class ProtoEnumConverter extends PrimitiveConverter {
     ...
     */
    private Map<Binary, Descriptors.EnumValueDescriptor> makeLookupStructure(
        Descriptors.FieldDescriptor enumFieldType) {
     ...
     * 
     * @throws org.apache.parquet.io.InvalidRecordException is there is no
     * corresponding value.
     */
    private Descriptors.EnumValueDescriptor translateEnumValue(Binary binaryValue) {
      ...
      if (protoValue == null) {
        ...
        String msg = "Illegal enum value \"" + binaryValue + "\"" + " in protocol buffer \"" + fieldType.getFullName()
  ...
  /**
   * This class unwraps the additional LIST wrapper and makes it possible to read
   * the underlying data and then convert it to protobuf.
   * <p>
   * Consider the following protobuf schema: message SimpleList { repeated int64
   * first_array = 1; }
   * <p>
   * A LIST wrapper is created in parquet for the above mentioned protobuf schema:
   * message SimpleList { optional group first_array (LIST) = 1 { repeated group
   * list { optional int32 element; } } }
   * <p>
   * The LIST wrappers are used by 3rd party tools, such as Hive, to read parquet
   * arrays. The wrapper contains a repeated group named 'list', itself containing
   * only one field called 'element' of the type of the repeated object (can be a
   * primitive as in this example or a group in case of a repeated message in
   * protobuf).
   */
