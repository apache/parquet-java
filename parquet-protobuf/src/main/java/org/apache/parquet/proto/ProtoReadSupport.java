...
public class ProtoReadSupport<T extends Message> extends ReadSupport<T> {
  ...
  public RecordMaterializer<T> prepareForRead(Configuration configuration, Map<String, String> keyValueMetaData, MessageType fileSchema, ReadContext readContext) {
    ...
    // If the parquet.schema.field.with.id is set on in the footer, map the schema with field id,
    // but users have the possibility to fallback to the old behavior of mapping field by name, by disabling explicitly the flag in the configuration.
    final boolean readFieldById = Boolean.valueOf(keyValueMetaData.get(ParquetMetadataConverter.PARQUET_SCHEMA_FIELD_WITH_ID))
      && configuration.getBoolean(ParquetMetadataConverter.PARQUET_SCHEMA_FIELD_WITH_ID, true);
    return new ProtoRecordMaterializer(requestedSchema, protobufClass, readFieldById);
  }
