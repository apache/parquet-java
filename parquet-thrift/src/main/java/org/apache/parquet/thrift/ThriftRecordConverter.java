...
public class ThriftRecordConverter<T> extends RecordMaterializer<T> {
  ...
  static class PrimitiveFieldHandler extends PrimitiveConverter {
    ...
    public PrimitiveFieldHandler(PrimitiveConverter delegate, final ThriftField field, List<TProtocol> events) {
      ...
      this.events = events;
      final byte thriftType = field.getType().getType() == ThriftTypeID.ENUM ? ThriftTypeID.I32.getThriftType() : // enums
                                                                                                                  // are
                                                                                                                  // serialized
                                                                                                                  // as
                                                                                                                  // I32
  ...
  /**
   * counts the instances created to use in List/Set/Map that need to inform of
   * the element count in the protocol
   */
  ...
  static class FieldEnumConverter extends PrimitiveConverter {
    ...
    public void addBinary(final Binary value) {
      ...
      if (id == null) {
        throw new ParquetDecodingException("Unrecognized enum value: " + value.toStringUsingUTF8() + " known values: "
            + enumLookup + " in " + this.field);
      }
  ...
  abstract class CollectionConverter extends GroupConverter {
    ...
    CollectionConverter(List<TProtocol> parentEvents, GroupType parquetSchema, ThriftField values) {
      ...
      if (ThriftSchemaConverter.isListElementType(repeatedType, values)) {
        ...
        this.elementConverter = new ElementConverter(parquetSchema.getName(), listEvents, repeatedType.asGroupType(),
            values);
  ...
  class ElementConverter extends GroupConverter {
    ...
    public ElementConverter(String listName, List<TProtocol> listEvents, GroupType repeatedType,
      ...
      for (int i = 0; i < schemaSize; i++) {
        ...
        if (schemaType.isPrimitive()) {
          converters[i] = new PrimitiveFieldHandler(
              newConverter(events, schemaType, matchingThrift).asPrimitiveConverter(), matchingThrift, events);
        } else {
          converters[i] = new GroupFieldhandler(newConverter(events, schemaType, matchingThrift).asGroupConverter(),
              matchingThrift, events);
        }
      }
    }
   ...
   * 
   * @param thriftReader the class responsible for instantiating the final object
   * and read from the protocol
   ...
   */
  @Deprecated
  public ThriftRecordConverter(ThriftReader<T> thriftReader, String name, MessageType requestedParquetSchema,
   ...
   */
  public ThriftRecordConverter(ThriftReader<T> thriftReader, String name, MessageType requestedParquetSchema,
      ThriftType.StructType thriftType, Configuration conf) {
    super();
    ...
    this.structConverter = new StructConverter(rootEvents, requestedParquetSchema,
        new ThriftField(name, (short) 0, Requirement.REQUIRED, thriftType));
  }
  ...
  private boolean hasMissingRequiredFieldInGroupType(GroupType requested, GroupType fullSchema) {
    for (Type field : fullSchema.getFields()) {
      ...
      if (requested.containsField(field.getName())) {
        ...
        // if a field is in requested schema and the type of it is a group type, then do
        // recursive check
