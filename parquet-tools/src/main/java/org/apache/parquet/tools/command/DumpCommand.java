...
public class DumpCommand extends ArgsOnlyCommand {
  ...
  static {
    OPTIONS = new Options();
    Option md = OptionBuilder.withLongOpt("disable-meta").withDescription("Do not dump row group and page metadata")
        .create('m');

    Option dt = OptionBuilder.withLongOpt("disable-data").withDescription("Do not dump column data").create('d');

    Option nocrop = OptionBuilder.withLongOpt("disable-crop")
        .withDescription("Do not crop the output based on console width").create('n');

    Option cl = OptionBuilder.withLongOpt("column")
        .withDescription("Dump only the given column, can be specified more than once").hasArg().create('c');
  ...
  public static void dump(PrettyPrintWriter out, ParquetMetadata meta, MessageType schema, Path inpath, boolean showmd,
      boolean showdt, Set<String> showColumns) throws IOException {
    Configuration conf = new Configuration();
    ...
    if (showdt) {
      ...
      for (ColumnDescriptor column : columns) {
        ...
        try {
          ...
          long offset = 1;
          freader = new ParquetFileReader(conf, meta.getFileMetaData(), inpath, blocks,
  ...
  public static void dump(final PrettyPrintWriter out, PageReadStore store, ColumnDescriptor column)
      throws IOException {
      ...
      page.accept(new Visitor<Void>() {
        ...
        public Void visit(DataPageV1 pageV1) {
          ...
          if (pageV1.getCrc().isPresent()) {
            try {
              out.format(" CRC:%s",
                  verifyCrc(pageV1.getCrc().getAsInt(), pageV1.getBytes().toByteArray()) ? "[verified]"
                      : "[PAGE CORRUPT]");
            } catch (IOException e) {
  ...
  public static void dump(PrettyPrintWriter out, ColumnReadStoreImpl crstore, ColumnDescriptor column, long page,
      long total, long offset) throws IOException {
    int dmax = column.getMaxDefinitionLevel();
    ColumnReader creader = crstore.getColumnReader(column);
    out.format("*** row group %d of %d, values %d to %d ***%n", page, total, offset,
        offset + creader.getTotalValueCount() - 1);
  ...
  private static PrettyPrintWriter prettyPrintWriter(boolean cropOutput) {
    PrettyPrintWriter.Builder builder = PrettyPrintWriter.stdoutPrettyPrinter().withAutoColumn()
        .withWhitespaceHandler(WhiteSpaceHandler.ELIMINATE_NEWLINES).withColumnPadding(1).withMaxBufferedLines(1000000)
        .withFlushOnTab();
